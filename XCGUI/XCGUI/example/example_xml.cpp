...
int APIENTRY _tWinMain(HINSTANCE hInstance,  HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	HXMLRES  hRes=XXmlRes_Load(L"xml\\resource.res");	//加载资源
	if(NULL==hRes)
	{
	...
	HWINDOW hWindow=XC_LoadXML_Window(L"xml\\layout.xml",hRes); //创建窗口
	if(NULL==hWindow)
	{
	...
	if(FALSE==XC_LoadXML_Style(hWindow,L"xml\\style.css",hRes))	//加载样式文件
	{
