...
BOOL CALLBACK View4_OnEventBtnClick(HELE hEle,HELE hEventEle)
{
	...
	if(hString)
	{
		MessageBox(XEle_GetHWnd(hEdit),XStr_GetBuffer(hString),L"获取选择内容",0);
...
int APIENTRY _tWinMain(HINSTANCE hInstance,  HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	//创建窗口
	hWindows=XWnd_CreateWindow(400,200,600,520,L"测试窗口",XC_SY_MINIMIZE| XC_SY_MAXIMIZE | XC_SY_CLOSE);
